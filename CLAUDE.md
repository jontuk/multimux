...
- **Terminal resize is arbitrated, not free.** All connections to a session share one tmux window. `Arbiter` gives size ownership to whichever connection most recently sent keyboard input; non-owners resize only their own attach PTY. Passing `resizeWindow: true` from a non-owner is a bug.
