...
- **Terminal resize is arbitrated, not free.** All connections to a session share one tmux window. For follow-input tiles, the browser reports deliberate keyboard, paste, pointer, wheel, or imperative input with an `active` resize; `Arbiter` gives size ownership to that client. Raw binary PTY data never proves human presence because xterm also emits automatic terminal-protocol replies through `onData`. Non-owners resize only their own attach PTY. Passing `resizeWindow: true` from a non-owner is a bug. Ownership is keyed on the `client=` id the tile puts on the PTY WebSocket (`web/src/clientId.ts`, stable per browser) and outlives that client's connections — and the session's last connection too, until `Arbiter.Prune` (reconcile tick) sees tmux has forgotten the session. An emptied session must not be up for grabs: a laptop that wakes for half a minute every hour on macOS dark wake reconnects with nobody at it, and its ordinary resize would otherwise claim the window and shrink every tile to the dormant machine's dims. Inheriting an *absent* owner's window therefore needs both `ownerGrace` and `presenceWindow` — a recent active resize from deliberate interaction, proving a human is at the claiming machine.
...
- **Sessions are reconciled, not trusted.** A row is written before its tmux session exists, so `Reconcile` (every 5s, plus at startup) honours `reconcileGrace` before declaring a running row dead. `CheckGitInfo` runs on its own 5s loop so a slow repo cannot delay it; auth-session sweeping runs hourly.
